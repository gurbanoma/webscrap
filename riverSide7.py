...
def TreatDate(daten):
    ...
    searchDate(daten, "DEED")
    searchDate(daten, "DEED OF TRUST")
    searchDate(daten, "RECONVEYANCE")
    
    dattmp = daten.replace('/', '')
    #create csv file
    filesp = []
    ...
    print ("Report completed for day:",daten, "result is in file:",filename1 )
...
def main():
    ...
    while not Q1.empty() :
        ...
        for i in range (maxTreats) :
            ...
            t = Thread (target=TreatDate, args=(daten,) )
            treats.append(t)
            t.start()
